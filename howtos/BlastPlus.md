...
Here, `$result` is a object. To obtain further results, use `next_result`:
...
$fac->rewind_results; while ($result = $fac->next_result) {
...
The return value is always a object on success, `undef` on failure.
...
Other collections (e.g., ) are valid. If a certain type does not work, please submit an [enhancement request](http://bugzilla.bioperl.org).

...
Here is some code which will read in a Tree from a file called "phyloxml_examples.xml" and produce a object which is stored in the variable $tree.
...
Once you have a Tree object you can do a number of things with it. You can use the methods required in to access the information on the nodes. For example, this script reads in phyloxml data and prints out the the node ids and bootstrap values.
...
my $newfile = "newfile.txt"; my $newio = Bio::TreeIO->new (-format => 'phyloxml', -file=>">$newfile"); $newio->write_tree($tree);
...
Or users can use the read_annotation method provided in to access the information directly through a XPath-like path string.
...
<sequence> elements are attached to the as a object. The sequence object can be retrieved using the sequence() method of . Even <sequence> elements without molecular sequence data are stored as SeqI objects with empty sequence..
...
<clade_relations> and <sequence_relations> are elements used in the <phylogeny> level to describe network connections between nodes or relationships between sequences ( e.g. orthology/paralogy). Bioperl reads the relation elements and finds the appropriate objects ( or ) corresponding to the description and attaches the to the object. When the tree is written, the s for each objects are translated into phylogeny level documentation.
